console.log('[DR] Service worker starting...');

// Prevent duplicate listener registration
if (self.DR_LISTENER_REGISTERED) {
	console.warn('[DR] Listener already registered, skipping...');
} else {
	...
	// Register listener (return true for async suggest)
	chrome.downloads.onDeterminingFilename.addListener((item, suggest) => {
		let suggestCalled = false;
		const safeSuggest = (arg) => {
			if (suggestCalled) {
				console.warn('[DR] Blocked duplicate suggest() call for id:', item.id);
				return;
			}
			suggestCalled = true;
			try { suggest(arg); } catch (e) { console.error('[DR] suggest() error:', e); }
		};

		const proceed = (cfg) => {
			log('Download:', item.id, item.filename);
			...
			if (!out) {
				...
				safeSuggest(); // Chrome default
				return;
			}

			out = sanitizeRelativePath(out);
			log('final suggest:', out);
			safeSuggest({ filename: out });
		};

		(configCache ? Promise.resolve(configCache) : loadConfig())
			.then(proceed)
			.catch(err => {
				console.error('[DR] Error in listener:', err);
				if (!suggestCalled) safeSuggest(); // Fallback
			});

		// Important: returning true keeps the event alive for async suggest
		return true;
	});

	// Mark listener as registered
	self.DR_LISTENER_REGISTERED = true;
	console.log('[DR] Listener registered successfully');
